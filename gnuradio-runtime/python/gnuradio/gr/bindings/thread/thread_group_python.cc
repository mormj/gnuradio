...
void bind_thread_group(py::module& m)
{
    ...
    py::class_<thread_group,boost::noncopyable_::noncopyable,
        std::shared_ptr<thread_group>>(m, "thread_group")

        .def(py::init<>())

        .def("create_thread",&thread_group::create_thread,
            py::arg("threadfunc") 
        )
        .def("add_thread",&thread_group::add_thread,
            py::arg("thrd") 
        )
        .def("remove_thread",&thread_group::remove_thread,
            py::arg("thrd") 
        )
